...
public class FormDAO implements IFormDAO {
   ...
   @Transactional
   public List<Form> listForms(Map<String, String> searchValue,
      ...
      if (searchValue.containsKey("from")
            && !searchValue.get("from").equals("")) {
         Date date = null;
         try {
            date = new SimpleDateFormat("yyyy-MM-dd").parse(searchValue
                  .get("from"));
      ...
      if (searchValue.containsKey("to") && !searchValue.get("to").equals("")) {
         ...
         try {
            date = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
                  .parse(searchValue.get("to") + " 23:59:59");
         } catch (ParseException e) {
      ...
      if (searchValue.containsKey("not_reg")
            && searchValue.get("not_reg") != null) {
         req.add(Restrictions.eq("is_registered", false));
      }
      if (searchValue.containsKey("user_id")
            && !searchValue.get("user_id").equals("")) {
      ...
      if (searchValue.containsKey("text")) {
         LogicalExpression restr = Restrictions.or(Restrictions.ilike(
               "surname_1", "%" + searchValue.get("text") + "%"), Restrictions
               .ilike("name_3", "%" + searchValue.get("text") + "%"));
         LogicalExpression restr2 = Restrictions.or(Restrictions.ilike(
               "name_3", "%" + searchValue.get("text") + "%"), Restrictions
               .ilike("identnum_11", "%" + searchValue.get("text") + "%"));
         LogicalExpression restr3 = Restrictions.or(
               Restrictions.or(restr, restr2),
               Restrictions.ilike("passnum_13", "%" + searchValue.get("text")
                     + "%"));
   ...
   public void removeForm(Integer id) {
      Form form = (Form) sessionFactory.getCurrentSession().get(Form.class, id);
      if (null != form) {
   ...
   public Form getForm(Integer id) {
      ...
      return (Form) sessionFactory.getCurrentSession().get(Form.class, id);
   }
}
